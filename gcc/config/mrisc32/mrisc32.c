...
static bool
mrisc32_is_imm21 (int ival)
{
  return ival >= -1048576 && ival <= 1048575;
}

static bool
mrisc32_is_imm21hi (int ival)
{
  unsigned int uval = (unsigned int)ival;
  return uval == (uval & 0xfffff800u);
}

static bool
mrisc32_is_imm21hio (int ival)
{
  unsigned int uval = (unsigned int)ival;
  return uval == ((uval & 0xfffff800u) | 0x000007ffu);
}
...
/* Generate instructions for loading an immediate integer value.  */
...
mrisc32_emit_load_immediate_int (rtx dst, int ival)
{
  ...
  if (mrisc32_is_imm21 (ival))
    {
      sprintf (insn_str, "ldi\t%%0, #%d", ival);
      output_asm_insn (insn_str, &dst);
    }
  else if (mrisc32_is_imm21hi (ival))
    {
      sprintf (insn_str, "ldhi\t%%0, #%d", ival);
      output_asm_insn (insn_str, &dst);
    }
  else if (mrisc32_is_imm21hio (ival))
    {
      sprintf (insn_str, "ldhio\t%%0, #%d", ival);
      output_asm_insn (insn_str, &dst);
    }
  else
    {
      const unsigned int uval = (unsigned int) ival;
      sprintf (insn_str, "ldhi\t%%0, #0x%x@hi", uval);
      output_asm_insn (insn_str, &dst);
      sprintf (insn_str, "or\t%%0, %%0, #0x%x@lo", uval);
      output_asm_insn (insn_str, &dst);
    }
}
...
mrisc32_emit_load_immediate_sym_plus_int (rtx dst, rtx sym_ref, int ival)
{
  char insn_str[512];
  char num_str[24];
  const char *orig_symbol = XSTR (sym_ref, 0);
  const char *symbol = (* targetm.strip_name_encoding) (orig_symbol);
  gcc_assert (symbol != NULL && (strlen (symbol) + 50) < sizeof (insn_str));

  /* addpchi dst, #sym+ival@pchi  */
  sprintf (num_str, "%s%d", ival > 0 ? "+" : "", ival);
  sprintf (insn_str, "addpchi\t%%0, #%s%s@pchi", symbol, ival ? num_str : "");
  output_asm_insn (insn_str, &dst);

  /* add dst, dst, #sym+ival+4@pclo  */
  ival += 4;
  sprintf (num_str, "%s%d", ival > 0 ? "+" : "", ival);
  sprintf (insn_str, "add\t%%0, %%0, #%s%s@pclo", symbol, ival ? num_str : "");
  output_asm_insn (insn_str, &dst);
}
...
mrisc32_compute_frame (void)
{
  ...
  offset += mrisc32_stack_align (cfun->machine->varargs_size);
#endif
			        ...
			        int *pretend_size, int no_rtl)
{
  ...
  *pretend_size = regs < 0 ? 0 : GET_MODE_SIZE (SImode) * regs;
  ...
  for (regno = *cum; regno <= MRISC32_S8; regno++)
    {
      rtx reg = gen_rtx_REG (SImode, regno);
      rtx slot = gen_rtx_PLUS (Pmode,
			       gen_rtx_REG (SImode, ARG_POINTER_REGNUM),
			       GEN_INT (UNITS_PER_WORD * (3 + (regno - MRISC32_S1))));
			      ...
			      const function_arg_info &arg)
{
  ...
  *cum = (*cum <= MRISC32_S8
	  ? *cum + ((3 + MRISC32_FUNCTION_ARG_SIZE (arg.mode, arg.type)) / 4)
	  : *cum);
}
